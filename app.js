...
/* icon by name (see icons.js); falls back to plain text so nothing renders blank */
const icon = (n, s) => (window.NXI && NXI.has(n) ? NXI.svg(n, s || 16) : esc(n || ""));
// Older saved data stored an emoji here; show the matching line icon instead.
const LEGACY_GLYPH = { "🌅": "sun", "💪": "dumbbell", "🚿": "droplet", "🎓": "book", "⚡": "code", "🍱": "utensils", "🤖": "cpu",
  "🏋": "dumbbell", "💻": "terminal", "🍽": "utensils", "🐙": "github", "🧠": "sparkles", "🌙": "moon", "💤": "moon",
  "📄": "file", "🌱": "sprout", "🚀": "rocket", "•": "dot", "◆": "dot" };
const glyph = (g, size) => {
  if (window.NXI && NXI.has(g)) return NXI.svg(g, size || 15);
  const k = String(g || "").replace(/\uFE0F/g, "");
  return window.NXI ? NXI.svg(LEGACY_GLYPH[k] || "dot", size || 15) : "";
};
...
function toast(msg, kind = "") {
  ...
  t.textContent = msg;              // plain text on purpose: messages can contain third-party names
  box.appendChild(t);
...
const DEFAULT_SCHEDULE = [
  { icon: "sun", label: "Wake Up",            st: "05:00", en: "05:10" },
  { icon: "dumbbell", label: "Morning Workout",    st: "05:10", en: "06:40" },
  { icon: "droplet", label: "Bath + Breakfast",   st: "06:40", en: "07:20" },
  { icon: "book", label: "DSA Lecture",        st: "07:20", en: "08:40" },
  { icon: "code", label: "DSA / LeetCode",     st: "08:40", en: "13:20" },
  { icon: "utensils", label: "Lunch",              st: "13:30", en: "14:10" },
  { icon: "code", label: "DSA / LeetCode",     st: "14:10", en: "15:10" },
  { icon: "cpu", label: "Project Dev",        st: "15:10", en: "17:20" },
  { icon: "dumbbell", label: "Evening Gym",        st: "17:35", en: "18:50" },
  { icon: "terminal", label: "Full Stack Session", st: "19:10", en: "20:00" },
  { icon: "utensils", label: "Dinner",             st: "20:00", en: "20:40" },
  { icon: "terminal", label: "Full Stack Session", st: "20:50", en: "21:20" },
  { icon: "github", label: "GitHub + LinkedIn",  st: "21:20", en: "21:45" },
  { icon: "sparkles", label: "AI / ML Concepts",   st: "21:45", en: "22:30" },
  { icon: "moon", label: "Wind Down",          st: "22:30", en: "23:00" },
  { icon: "moon", label: "Sleep",              st: "23:00", en: "05:00" },
];

const DEFAULT_TASKS = [
  { id: "wake",    icon: "sun", label: "Wake up at 5:00 AM",        cat: "Routine" },
  { id: "wm",      icon: "dumbbell", label: "Morning workout",           cat: "Fitness" },
  { id: "dsa_lec", icon: "book", label: "DSA lecture",               cat: "Study" },
  { id: "dsa",     icon: "code", label: "DSA / LeetCode solving",    cat: "Study" },
  { id: "proj",    icon: "cpu", label: "Project development",       cat: "Build" },
  { id: "gym",     icon: "dumbbell", label: "Evening gym",               cat: "Fitness" },
  { id: "fs",      icon: "terminal", label: "Full stack session",        cat: "Study" },
  { id: "github",  icon: "github", label: "GitHub commit + LinkedIn",  cat: "Career" },
  { id: "ai",      icon: "sparkles", label: "AI / ML revision",          cat: "Study" },
  { id: "sleep",   icon: "moon", label: "Sleep by 11:00 PM",         cat: "Routine" },
];

const DEFAULT_PROJECTS = [
  { id: uid(), emoji: "cpu", name: "AI Desktop Assistant", desc: "JARVIS-style AI for Windows",
  ...
  { id: uid(), emoji: "file", name: "AI Resume Ranker", desc: "NLP resume scoring system",
  ...
  { id: uid(), emoji: "sprout", name: "Carbon Track + Time Bank", desc: "Sustainability gamification platform",
...
/* ============================================================
   STATE  +  PERSISTENCE
   ============================================================ */
/* Data is namespaced per signed-in user: NEXUS.start(user) switches the key. The old
   single-user key is adopted once, by the first account that signs in on this browser. */
const LEGACY_KEY = "nexus-os:v2";
let LS_KEY = LEGACY_KEY;
...
function save() {
  ...
  updateChrome();
  if (window.NEXUS && NEXUS.onSave) NEXUS.onSave();
}
...
/* tracker rows the user has actually applied to (not just saved from the feed) */
function appliedJobs() { return S.jobs.filter((j) => j.pipeline !== "saved" && j.stage !== "Found"); }
...
function applyTheme() {
  ...
  /* shared with the landing + sign-in pages, which have no app state of their own */
  try {
    if (S.settings.theme === "light" || S.settings.theme === "dark") localStorage.setItem("nexus-theme", S.settings.theme);
    else localStorage.removeItem("nexus-theme");
  } catch (e) { /* storage blocked */ }
  const b = el("themeBtn");
  if (b) b.innerHTML = icon(S.settings.theme === "system" ? "monitor" : S.settings.theme === "dark" ? "moon" : "sun", 15);
}
...
const stat = (label, value, sub, opts = {}) => `
  <div class="stat">
    ${opts.icon ? `<span class="stat-ico" style="color:${opts.color || "var(--faint)"}">${icon(opts.icon, 16)}</span>` : ""}
...
const empty = (ico, title, sub) =>
  `<div class="empty"><div class="e-ico">${icon(ico, 28)}</div><div class="e-title">${esc(title)}</div><div class="e-sub">${esc(sub)}</div></div>`;
...
async function pushSolve(id, opts = {}) {
  ...
  save();
  if (NEXUS.onPushed && !opts.skipReadme) setTimeout(() => NEXUS.onPushed(id), 0);
...
/* Prompts live in prompts.js (shared with the AI proxy). Here: labels + the data each one sends. */
const COACH = {
  brief: { label: "Daily briefing",      icon: "file" },
  next:  { label: "What to solve next",  icon: "target" },
  retro: { label: "Weekly retro",        icon: "calendar" },
  job:   { label: "Job search advice",   icon: "briefcase" },
};
function coachData(kind) {
  if (kind === "next") {
    return { thin: solveStats().thinTopics, week: last7(activityCal()).reduce((a, b) => a + b, 0), pool: nextProblemPool() };
  }
  if (kind === "job") {
    return {
      funnel: NEXUS.funnel ? NEXUS.funnel() : null,
      recent: NEXUS.recentApps ? NEXUS.recentApps() : [],
      profile: NEXUS.profileBrief ? NEXUS.profileBrief() : null,
      solved: { total: solvedTotal(), byTopic: solveStats().byTopic },
    };
  }
...
async function reviewSolution() {
  ...
  try {
    const text = await NXAI.run("review", { problem: prob, code });
...
function scoreJob(job, f) {
  ...
  let skill = 0, hits = [];
  /* f.weights (built from the profile) scales each skill: core skills count more than listed ones */
  const wOf = (k) => (f.weights && f.weights[String(k).trim().toLowerCase()]) || 1;
  for (const k of f.include) {
    if (hasTerm(title, k)) { skill += 3 * wOf(k); hits.push(k.trim()); }
    else if (hasTerm(hay, k)) { skill += 1 * wOf(k); hits.push(k.trim()); }
  }
  if (skill === 0) return -1;

  let score = skill;
  for (const lv of f.levels) if (hasTerm(title, lv)) { score += 4; hits.push(lv.trim()); break; }
  /* locations from the profile; with none set (or any Indian city listed) keep the India boost */
  const wantLocs = (f.locations || []).filter((l) => !/^(remote|anywhere)$/i.test(l));
  const wantsIndia = !wantLocs.length || wantLocs.some((l) => IN_CITY.test(String(l).toLowerCase()));
  if (wantsIndia && IN_CITY.test(loc)) score += 3;
  else if (wantLocs.some((l) => String(l).length > 2 && loc.includes(String(l).toLowerCase()))) score += 3;
  else if (f.remoteOk !== false && /\b(remote|anywhere|worldwide)\b/.test(loc)) score += 1;
...
const NAV = [
  { group: "Overview", items: [
    ...
    { id: "daily", label: "Daily Tracker", icon: "checks" },
  ]},
  { group: "Tracking", items: [
    { id: "leetcode", label: "LeetCode", icon: "code" },
    { id: "solves", label: "Solutions", icon: "terminal" },
  ...
  { group: "Career", items: [
    { id: "jobfeed", label: "Job Feed", icon: "search" },
    { id: "jobs", label: "Job Tracker", icon: "columns" },
  ]},
  { group: "System", items: [
...
const TITLES = {}, GROUPS = {};
function indexNav() { NAV.forEach((g) => g.items.forEach((i) => { TITLES[i.id] = i.label; GROUPS[i.id] = g.group; })); }
indexNav();
  ...
  el("nav").innerHTML = NAV.map((g) => `
    ...
    ${g.items.map((i) => `
      <button class="nav-item" data-action="nav" data-view="${i.id}">
        <span class="ni-icon">${icon(i.icon, 16)}</span>${i.label}
      </button>`).join("")}
  `).join("");
}
function go(view) {
  ...
  el("pageEyebrow").textContent = GROUPS[view] || "Overview";
...
ROUTES.overview = function () {
  ...
  ${!S.settings.leetcode && !S.settings.github ? `
    <div class="card" style="border-color:color-mix(in srgb,var(--accent) 30%,transparent);background:var(--accent-weak)">
      <div class="card-head" style="margin-bottom:8px"><div class="card-title" style="display:flex;align-items:center;gap:8px"><span style="color:var(--accent);display:inline-flex">${icon("user", 16)}</span>Connect your accounts</div></div>
  ...
  ${cur ? `<div class="now-strip">
    ...
    <span class="now-ico">${glyph(cur.icon, 15)}</span>
  ...
  <div class="grid g-4" style="margin-bottom:14px">
    ${stat("Problems Solved", solved, `of ${goal} goal · ${pct(solved, goal)}%`, { icon: "code", color: solved ? "var(--accent)" : "var(--faint)" })}
    ${stat("Day Streak", streak || 0, streak ? "keep it alive" : "solve today to start", { icon: streak ? "flame" : "dot", color: streak ? "var(--amber)" : "var(--faint)" })}
    ${stat("Tasks Today", tt.done + "/" + tt.total, tt.pct + "% complete", { icon: "check", color: tt.done ? "var(--green)" : "var(--faint)" })}
    ${stat("Solved Today", ps.today, ps.today >= 5 ? "target hit" : "target: 5/day", { icon: "chart", color: ps.today ? "var(--accent)" : "var(--faint)" })}
  </div>
  ...
  <div class="grid g-2" style="margin-bottom:14px">
    ...
    <div class="card">
      ...
      ${week.some((d) => d.value) ? barChart(week) : empty("bars", "No activity yet", "Sync LeetCode or log practice to fill this in")}
    </div>
  </div>
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    ${Object.keys(cal).length ? heatmap(cal, { weeks: 26 }) : empty("calendar", "Nothing here yet", "Your activity graph lights up as you solve")}
  </div>

  ${(() => {
    const ssd = solveStats();
    const bits = [];
    if (ssd.needCode) bits.push(`<button class="btn btn-sm" data-action="nav" data-view="solves">${icon("edit", 13)} ${ssd.needCode} synced solve${ssd.needCode === 1 ? "" : "s"} need code</button>`);
    if (ssd.readyToPush && S.settings.ghToken) bits.push(`<button class="btn btn-sm btn-primary" data-action="solve-backfill">${icon("upload", 13)} Push ${ssd.readyToPush} to GitHub</button>`);
    if (aiReady()) bits.push(`<button class="btn btn-sm" data-action="coach-run" data-kind="brief">${icon("sparkles", 13)} Daily briefing</button>`);
  ...
  ${S.gh ? `<div class="card">
    ...
    ${S.gh.contributions.length ? heatmap(calFromContrib(S.gh.contributions), { weeks: 26, palette: ["var(--gh0)", "var(--gh1)", "var(--gh2)", "var(--gh3)", "var(--gh4)"] }) : empty("branch", "No contribution data", "")}
  </div>` : ""}
  `;
};
...
ROUTES.schedule = function () {
  ...
  <div class="card" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
    ...
    ${cur ? `<div style="text-align:right"><div class="section-label" style="margin:0 0 4px">Active block</div>
      <div style="font-weight:700;display:flex;align-items:center;gap:7px;justify-content:flex-end">${glyph(cur.icon, 15)} ${esc(cur.label)}</div>
    ...
    <div class="stack" style="gap:4px">
      ${S.schedule.map((s, i) => {
        ...
        return `<div class="sched-item ${st}">
          <span class="sched-ico">${glyph(s.icon, 15)}</span>
...
ROUTES.daily = function () {
    ...
    <div class="stack" style="gap:2px">
      ${S.taskDefs.map((t) => {
        ...
        return `<div class="row-item ${done ? "done" : ""}" data-action="task-toggle" data-id="${t.id}">
          ...
          <span class="ri-ico">${glyph(t.icon, 14)}</span>
  ...
  <div class="card">
    ...
    <div class="grid g-3" style="gap:8px;margin-bottom:12px">
      <button class="btn" data-action="practice-add" data-diff="easy">${icon("plus", 12)} Easy</button>
      <button class="btn" data-action="practice-add" data-diff="medium">${icon("plus", 12)} Medium</button>
      <button class="btn" data-action="practice-add" data-diff="hard">${icon("plus", 12)} Hard</button>
    </div>
...
ROUTES.leetcode = function () {
  ...
  <div class="grid g-4" style="margin-bottom:14px">
    ${stat("Total Solved", solved, "goal " + goal, { icon: "code", color: "var(--accent)" })}
    ${stat("Easy", e, "", { icon: "circle", color: "var(--green)" })}
    ${stat("Medium", m, "", { icon: "circle", color: "var(--amber)" })}
    ${stat("Hard", h, "", { icon: "circle", color: "var(--red)" })}
  </div>

  ${lc ? `<div class="grid g-4" style="margin-bottom:14px">
    ${stat("Ranking", lc.ranking ? "#" + lc.ranking.toLocaleString() : "—", "global", { icon: "trophy" })}
    ${stat("Acceptance", lc.acceptance != null ? lc.acceptance + "%" : "—", "", { icon: "check" })}
    ${stat("Total Questions", lc.totalQ ? lc.totalQ.toLocaleString() : "—", "on LeetCode", { icon: "hash" })}
    ${stat("Streak", streakFrom(cal) + "d", "consecutive", { icon: "flame", color: "var(--amber)" })}
  </div>` : ""}
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    ${Object.keys(cal).length ? heatmap(cal, { weeks: 30 }) : empty("calendar", "No submissions yet", "Sync your account or log practice")}
  </div>

  ${lc && lc.recent.length ? `<div class="card">
    <div class="card-head"><div class="card-title">Recent submissions</div></div>
    <div class="table-wrap"><table class="mini-table"><thead><tr><th>Problem</th><th>Result</th><th>Lang</th><th>When</th></tr></thead><tbody>
      ${lc.recent.map((r) => `<tr>
...
ROUTES.solves = function () {
  ...
  <div class="grid g-4" style="margin-bottom:14px">
    ${stat("Logged", ss.count, "solve entries", { icon: "terminal", color: ss.count ? "var(--accent)" : "var(--faint)" })}
    ${stat("On GitHub", ss.pushed, ss.count ? Math.round(ss.pushed / ss.count * 100) + "% of log" : "none yet", { icon: "upload", color: ss.pushed ? "var(--green)" : "var(--faint)" })}
    ${stat("Ready to push", ss.readyToPush, "code saved here", { icon: "dot", color: ss.readyToPush ? "var(--amber)" : "var(--faint)" })}
    ${stat("Older solves", ss.needCode, "no code stored", { icon: "clock", color: "var(--faint)" })}
  </div>
    ...
    ${connected
      ? `<div class="mut" style="font-size:12.5px">Committing to <strong>${esc(st.ghRepo)}</strong>. Each solve writes <code>solutions/&lt;topic&gt;/&lt;id&gt;-&lt;slug&gt;.&lt;ext&gt;</code> plus a README index — one commit each.</div>
         <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap">
           <button class="btn btn-primary" data-action="solve-backfill">${icon("upload", 13)} Push all with code (${ss.readyToPush})</button>
           <button class="btn" data-action="repo-pull" title="Import what the browser extension already pushed">${icon("download", 13)} Pull from repo</button>
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    ${!list.length ? empty("terminal", "No solves logged yet", "Sync LeetCode to auto-import, or log one manually")
...
function solveRow(s, connected) {
  ...
  const state = s.pushed
    ? `<a class="badge b-green" href="${esc(s.pushUrl || "#")}" target="_blank" rel="noopener" data-stop="1">pushed ✓</a>`
  ...
  return `<div class="solve-row${open ? " open" : ""}${s.pushed ? " is-pushed" : ""}">
    <div class="solve-head" data-action="solve-expand" data-id="${s.id}">
      ...
      <a class="sh-title" href="https://leetcode.com/problems/${esc(s.slug)}/" target="_blank"
         rel="noopener" data-stop="1" title="${esc(s.title)}">${esc(s.title)}</a>
    ...
    ${open ? `<div class="solve-body">
      ...
      ${s.review ? `<div class="review-box">
        <div class="review-head"><span class="badge b-accent">${icon("sparkles", 11)} AI review</span>
          <span class="faint" style="font-size:11px">${ago(new Date(s.review.date).getTime())}</span>
          <button class="btn btn-sm btn-ghost right" data-action="review-clear" data-id="${s.id}">Dismiss</button></div>
        <div class="md-body">${mdToHtml(s.review.text)}</div></div>` : ""}
      <label class="field-label">Solution code</label>
      ...
      <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
        ...
        <button class="btn btn-ghost" data-action="solve-advise" data-id="${s.id}" ${hasCode && aiReady() ? "" : "disabled"} title="${hasCode ? "Code feedback plus job-search advice" : "Needs stored code"}">${icon("sparkles", 13)} What can I do better?</button>
...
ROUTES.github = function () {
  ...
  <div class="card">
    ...
    <div style="margin-top:14px">${empty("branch", "Not connected yet", "Add your username to pull repos, stars and contributions")}</div>
  </div>`;
  ...
  <div class="grid g-4" style="margin:14px 0">
    ${stat("Contributions", g.contribTotal || 0, "past year", { icon: "branch", color: g.contribTotal ? "var(--green)" : "var(--faint)" })}
    ${stat("Public repos", p.publicRepos || 0, "on your profile", { icon: "box", color: "var(--accent)" })}
    ${stat("Stars earned", g.stars || 0, "across your repos", { icon: "star", color: g.stars ? "var(--amber)" : "var(--faint)" })}
    ${stat("Followers", p.followers || 0, `following ${p.following || 0}`, { icon: "user", color: "var(--faint)" })}
  </div>
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    ${g.contributions.length
      ...
      : empty("branch", "No contribution data", "The contributions proxy may be down — sync again later")}
  </div>
  ...
  <div class="grid g-2" style="margin-bottom:14px">
      ...
      ${g.topRepos.length ? g.topRepos.map((r) => `
        <div class="list-line" style="display:block">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:baseline">
            ...
            <span class="faint tabnum" style="font-size:11.5px;display:inline-flex;align-items:center;gap:4px">${icon("star", 12)} ${r.stars}</span>
          </div>
        ...
        </div>`).join("") : empty("box", "No repos yet", "Push a project to see it here")}
    </div>
    ...
    <div class="card">
      ...
      ${g.langs.length ? g.langs.map(([lang, n]) => `
        ...
        </div>`).join("") : empty("layers", "No languages detected", "")}
    </div>
  </div>
  ...
  <div class="card">
    ...
    ${g.events.length ? g.events.map((e) => `
      ...
      </div>`).join("") : empty("clock", "No recent public activity", "")}
  </div>
  `;
};
...
ROUTES.coach = function () {
  ...
  <div class="card" style="margin-bottom:14px;border-color:color-mix(in srgb,var(--amber) 30%,transparent);background:var(--amber-weak)">
    <div class="card-head" style="margin-bottom:8px"><div class="card-title" style="display:flex;align-items:center;gap:8px"><span style="color:var(--amber);display:inline-flex">${icon("key", 16)}</span>Turn on the AI coach</div></div>
    <p class="mut" style="font-size:13px;margin-bottom:12px">The coach runs on Groq. Add a free key in Settings to switch it on — everything below stays disabled until then.</p>
    <button class="btn btn-primary" data-action="nav" data-view="settings">Open Settings →</button>
  </div>` : ""}
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    <div class="wrap">
      ${Object.entries(COACH).map(([k, c]) => `
        <button class="btn btn-sm ${hasKey ? "btn-primary" : "btn-ghost"}" data-action="coach-run" data-kind="${k}"${hasKey ? "" : " disabled"}>${icon(c.icon, 14)} ${esc(c.label)}</button>`).join("")}
    </div>
    <div id="coachOut"></div>
  </div>
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    <div style="margin-top:10px">
      <button class="btn btn-primary" data-action="coach-review"${hasKey ? "" : " disabled"}>${icon("sparkles", 13)} Review my solution</button>
    </div>
  ...
  <div class="card">
    ...
    ${!h.length ? empty("sparkles", "Nothing yet", "Run a briefing or a retro and it lands here") : h.map((x, i) => `
      <div class="list-line" style="display:block">
...
/* ---------- Skills ---------- */
const SKILL_GROUPS = {
  fullstack: { label: "Full-Stack Development", icon: "box" },
  aiml:      { label: "AI / ML",                icon: "cpu" },
};
ROUTES.skills = function () {
  ...
  const meta = (g) => SKILL_GROUPS[g] || { label: titleCase(g), icon: "dot" };
  ...
  <div class="grid g-4" style="margin:14px 0">
    ${stat("Overall", avg(all) + "%", "across every skill", { icon: "layers", color: avg(all) ? "var(--accent)" : "var(--faint)" })}
    ...
    ${stat("Solid", all.filter((s) => (s.v || 0) >= 70).length, `of ${all.length} at 70%+`, { icon: "check", color: "var(--green)" })}
  </div>

  ${groups.map((g) => `
    <div class="card" style="margin-bottom:14px">
      <div class="card-head">
        <div class="card-title" style="display:flex;align-items:center;gap:8px"><span style="color:var(--accent);display:inline-flex">${icon(meta(g).icon, 16)}</span>${esc(meta(g).label)}</div>
      ...
      ${S.skills[g].length ? S.skills[g].map((s, i) => `
        ...
        </div>`).join("") : empty("dot", "No skills in this group", "Add one below")}
...
ROUTES.projects = function () {
  ...
  <div class="grid g-3" style="margin:14px 0">
    ${stat("Projects", ps.length, "in the portfolio", { icon: "box", color: ps.length ? "var(--accent)" : "var(--faint)" })}
    ${stat("Tasks done", doneTasks + "/" + totalTasks, pct(doneTasks, totalTasks) + "% complete", { icon: "check", color: doneTasks ? "var(--green)" : "var(--faint)" })}
    ${stat("Shipped", shipped, "fully complete", { icon: "rocket", color: shipped ? "var(--green)" : "var(--faint)" })}
  </div>

  ${!ps.length ? `<div class="card">${empty("box", "No projects yet", "Add one — a shipped project beats a hundred tutorials")}</div>`
  : `<div class="stack">${ps.map((p) => {
      const d = doneOf(p), t = p.tasks.length;
      return `
      <div class="card">
        <div class="card-head">
          <div class="card-title" style="display:flex;align-items:center;gap:8px">${glyph(p.emoji, 16)} ${esc(p.name)}</div>
...
ROUTES.jobfeed = function () {
  ...
  const matched = feedFilterTab(all, tab, f);
  const jobs = matched.slice(0, _feedLimit);
  ...
  <div class="grid g-4" style="margin-bottom:14px">
    ${stat("Matches", counts.all, "after your filters", { icon: "user", color: counts.all ? "var(--accent)" : "var(--faint)" })}
    ${stat("New", counts.new, "since last check", { icon: "sparkles", color: counts.new ? "var(--green)" : "var(--faint)" })}
    ${stat("In India", counts.india, "on-site or hybrid", { icon: "pin", color: counts.india ? "var(--accent)" : "var(--faint)" })}
    ${stat("Entry level", counts.entry, "intern / new grad", { icon: "sprout", color: counts.entry ? "var(--green)" : "var(--faint)" })}
  </div>
  ...
  <div class="card" style="margin-bottom:14px">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
      ...
      <div class="wrap">
        <button class="btn btn-sm btn-primary" data-action="feed-refresh">${icon("refresh", 13)} Fetch jobs</button>
        ${counts.new ? `<button class="btn btn-sm btn-ghost" data-action="feed-seen">Mark all seen</button>` : ""}
        <button class="btn btn-sm btn-ghost" data-action="feed-sources">${_showSources ? "Close" : `${icon("sliders", 13)} Sources &amp; filters`}</button>
      </div>
    </div>
  ...
  ${!all.length
    ? `<div class="card">${empty("search", "No jobs loaded yet", "Press “Fetch jobs” to pull live postings from your boards")}</div>`
    : !jobs.length
    ? `<div class="card">${empty("check", "Nothing in this tab", "You're caught up here — try “All”, or loosen your filters under Sources & filters")}</div>`
    : `<div class="stack">${jobs.map(feedCard).join("")}</div>
       ${matched.length > jobs.length ? `<div style="text-align:center;margin-top:14px">
         <button class="btn" data-action="feed-more">Show ${Math.min(40, matched.length - jobs.length)} more <span class="faint">- ${matched.length - jobs.length} left</span></button></div>` : ""}`}
  `;
};
...
ROUTES.jobs = function () {
  ...
  <div class="grid g-4" style="margin-bottom:14px">
    ${stat("Applied", total, "total sent", { icon: "send", color: total ? "var(--accent)" : "var(--faint)" })}
    ${stat("Responses", resp, "interviews / OA", { icon: "message", color: resp ? "var(--green)" : "var(--faint)" })}
    ${stat("Rejected", rej, "data, not failure", { icon: "bars", color: "var(--faint)" })}
    ${stat("Response Rate", rr + "%", "industry avg ~10%", { icon: "chart", color: rr > 10 ? "var(--green)" : "var(--faint)" })}
  </div>
  ...
  ${!total ? `<div class="card">${empty("target", "No applications yet", "Track every send — momentum compounds")}</div>` : `
  <div class="card">
...
ROUTES.analytics = function () {
  ...
  const rows = [
    ...
    ["Applications", appliedJobs().length, appliedJobs().filter((x) => x.status === "response").length + " responses", "var(--accent)"],
  ];
  ...
  <div class="grid g-4" style="margin-bottom:14px">
    ${stat("Solved", solved, pct(solved, goal) + "% of goal", { icon: "code", color: "var(--accent)" })}
    ${stat("Streak", streak + "d", "current", { icon: "flame", color: streak ? "var(--amber)" : "var(--faint)" })}
    ${stat("FS Mastery", fsAvg + "%", "full stack", { icon: "code", color: fsAvg ? "var(--accent)" : "var(--faint)" })}
    ${stat("AI/ML Mastery", aiAvg + "%", "avg", { icon: "cpu", color: aiAvg ? "var(--green)" : "var(--faint)" })}
  </div>

  <div class="grid g-2" style="margin-bottom:14px">
    <div class="card">
      <div class="card-head"><div class="card-title">Last 7 days</div></div>
      ${week.some((d) => d.value) ? barChart(week) : empty("chart", "No data yet", "Log or sync problems")}
    </div>
    <div class="card">
  ...
  <div class="card">
    ...
    ${[
      [`Problem goal (${goal})`, solved > 0 ? `~${Math.ceil((goal - solved) / 5)} days` : "—", "zap"],
      ["Full stack → 80%", fsAvg > 0 ? `~${Math.max(0, Math.ceil((80 - fsAvg) * 1.5))} days` : "—", "code"],
      ["AI/ML → 70%", aiAvg > 0 ? `~${Math.max(0, Math.ceil((70 - aiAvg) * 2))} days` : "—", "cpu"],
      ["10 applications", `${Math.max(0, 10 - appliedJobs().length)} to go`, "target"],
    ].map((r) => `<div class="list-line"><span style="color:var(--accent);display:inline-flex">${icon(r[2], 15)}</span><span class="ri-text">${r[0]}</span>
...
ROUTES.settings = function () {
  ...
  <div class="page-sub">Your account, connections, goals and data.</div>

  ${NEXUS.user ? `<div class="card" style="margin-bottom:14px">
    <div class="card-head"><div class="section-label" style="margin:0">Account</div>
      ${NEXUS.user.local ? `<span class="badge b-amber">local mode</span>` : NEXUS.user.verified ? `<span class="badge b-green">verified</span>` : `<span class="badge b-amber">email not verified</span>`}</div>
    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
      <span class="uc-avatar" style="width:38px;height:38px;font-size:18px">${esc((NEXUS.user.name || "?").trim()[0] || "?").toUpperCase()}</span>
      <div style="flex:1;min-width:160px"><div style="font-weight:600">${esc(NEXUS.user.name || "")}</div>
        <div class="faint" style="font-size:12px">${NEXUS.user.local ? "No sign-in — data stays in this browser only" : esc(NEXUS.user.email)}</div></div>
      ${!NEXUS.user.local && !NEXUS.user.verified ? `<button class="btn btn-sm" data-action="resend-verify">Resend verification</button>` : ""}
      <button class="btn btn-sm" data-action="sign-out">${icon("logout", 13)} ${NEXUS.user.local ? "Exit" : "Sign out"}</button>
    </div>
  </div>` : ""}
  ...
  ${NEXUS.aiSettingsCard ? NEXUS.aiSettingsCard() : ""}
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn" data-action="data-export">${icon("download", 13)} Export JSON</button>
      <button class="btn" data-action="data-import">${icon("upload", 13)} Import JSON</button>
...
function updateChrome() {
  ...
  if (nu) {
    if (streak > 0) {
      fl.innerHTML = icon("flame", 20); fl.style.color = "var(--amber)"; nu.textContent = streak + " day" + (streak > 1 ? "s" : "");
      su.textContent = "keep it going"; nu.style.color = "var(--amber)";
    } else {
      fl.innerHTML = icon("zap", 20); fl.style.color = "var(--accent)"; nu.textContent = "No streak yet";
  ...
  const uc = el("userChip");
  if (uc && NEXUS.user) {
    const u = NEXUS.user, nm = u.name || u.email || "Local mode";
    uc.innerHTML = `<span class="uc-avatar">${esc(nm.trim()[0] || "?").toUpperCase()}</span>
      <span class="uc-text"><span class="uc-name">${esc(nm)}</span><span class="uc-sub">${u.local ? "local mode" : esc(u.email || "")}</span></span>
      <button class="icon-btn" data-action="sign-out" title="${u.local ? "Back to the landing page" : "Sign out"}" aria-label="Sign out">${icon("logout", 15)}</button>`;
  }
  const lsn = el("lastSyncNote");
  if (lsn) {
    const last = Math.max(S.lc ? S.lc.lastSync : 0, S.gh ? S.gh.lastSync : 0);
    lsn.textContent = "LeetCode/GitHub " + (last ? "synced " + ago(last) : "not synced yet");
  }
}

/* ============================================================
   EXTENSION SURFACE
   Feature files (profile.js, tracker.js, ai.js ...) load after this file and
   plug in here: routes, nav items, click/change/input handlers, start hooks.
   ============================================================ */
const NEXUS = window.NEXUS = {
  user: null,
  actions: {}, changes: {}, inputs: {}, onStart: [],
  ROUTES, ROUTES_AFTER, NAV,
  get S() { return S; },
  save, toast, rerender, go, esc, icon, stat, empty, progress, ring, barChart,
  iso, addDays, ago, uid, clone, pct, clamp, titleCase, slugify, mdToHtml, fetchJSON,
  streakFrom, activityCal, solveStats, todayTaskState, practiceStats, solvedTotal,
  el, $, $$, b64enc, scoreJob, normTerm, hasTerm, IN_CITY,
  get current() { return current; },
  saveDebounced: (...a) => saveDebounced(...a),
  refreshFeed: (...a) => refreshFeed(...a),
  groqChat: (...a) => groqChat(...a),
  pushSolve: (...a) => pushSolve(...a),
  reload() { loadState(); rerender(); },
  get storageKey() { return LS_KEY; },
};
...
document.addEventListener("click", (ev) => {
  if (ev.target.closest("a[data-stop]")) return;       // links inside clickable rows: just navigate, don't trigger the row
  ...
  switch (a) {
    ...
    case "task-add": {
      ...
      if (label) { S.taskDefs.push({ id: uid(), icon: "dot", label: label.trim(), cat: "Custom" }); save(); rerender(); }
      break;
    }
    case "task-del":
    ...
    /* schedule */
    case "sched-add":
      S.schedule.push({ icon: "dot", label: "New block", st: "12:00", en: "13:00" }); save(); rerender(); break;
    case "sched-del":
    ...
    case "proj-add": {
      ...
      if (!n) break;
      S.projects.push({ id: uid(), emoji: "rocket", name: n.trim(), desc: "New project", tech: [], tasks: [] });
      save(); rerender(); break;
    }
    case "proj-del":
    ...
    case "feed-tab": _feedTab = D.t; _feedLimit = 40; rerender(); break;
    case "feed-more": _feedLimit += 40; rerender(); break;
    case "feed-agg": {
    ...
    case "feed-track": {
      ...
      S.jobs.unshift({
        ...
        jobId: j.id, loc: j.loc || "", pipeline: "saved",
      });
      S.jobfeed.tracked[j.id] = true;
      S.jobfeed.seen[j.id] = true;
      save(); toast(`Saved ${j.title} to your tracker`, "ok"); rerender();
      break;
    }
    ...
    case "settings-save": {
      ...
      if (gc("setAutoAdvise") !== undefined) S.settings.autoAdvise = gc("setAutoAdvise");
    ...
    case "resend-verify":
      if (window.NexusAuth) NexusAuth.resendVerification().then(() => toast("Verification email sent", "ok")).catch((e) => toast(NexusAuth.friendly(e), "err"));
      break;
    ...
    case "data-reset":
      ...
      break;
    default:
      if (NEXUS.actions[a]) NEXUS.actions[a](D, t, ev);
  }
});
...
document.addEventListener("input", (ev) => {
  ...
  if (a === "skill-range") {
    ...
    saveDebounced();
  } else if (NEXUS.inputs[a]) NEXUS.inputs[a](t, D, ev);
});
document.addEventListener("change", (ev) => {
  ...
  else if (NEXUS.changes[a]) NEXUS.changes[a](t, D, ev);
});
let sdTimer = null;
function saveDebounced() { clearTimeout(sdTimer); sdTimer = setTimeout(save, 400); }

/* ============================================================
   VISUAL FLOURISHES  (glare under the cursor, scroll progress)
   ============================================================ */
const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
const finePointer = window.matchMedia("(pointer: fine)");
document.addEventListener("mousemove", (ev) => {
  if (reducedMotion.matches || !finePointer.matches) return;
  const c = ev.target.closest && ev.target.closest(".card, .stat, .job-card");
  if (!c) return;
  const r = c.getBoundingClientRect();
  c.style.setProperty("--glare-x", ((ev.clientX - r.left) / r.width) * 100 + "%");
  c.style.setProperty("--glare-y", ((ev.clientY - r.top) / r.height) * 100 + "%");
}, { passive: true });
...
/* ============================================================
   START  - app.html calls NEXUS.start(user) once the visitor is signed in
   ============================================================ */
let started = false;
function adoptLegacy(uid) {
  try {
    if (localStorage.getItem(LS_KEY) || !localStorage.getItem(LEGACY_KEY)) return;
    const owner = localStorage.getItem(LEGACY_KEY + ":adopted");
    if (owner && owner !== uid) return;              // someone else already claimed it
    localStorage.setItem(LS_KEY, localStorage.getItem(LEGACY_KEY));
    localStorage.setItem(LEGACY_KEY + ":adopted", uid);
  } catch (e) { /* storage blocked */ }
}

function start(user) {
  NEXUS.user = user || null;
  if (user && user.uid) { LS_KEY = "nexus-os:v3:" + user.uid; adoptLegacy(user.uid); }
  loadState();
  applyTheme();
  buildNav();
  updateChrome();
  ...
  autoPullRepo();
  autoSync();
  NEXUS.onStart.forEach((fn) => { try { fn(); } catch (e) { console.warn("onStart failed", e); } });
}
NEXUS.start = start;
